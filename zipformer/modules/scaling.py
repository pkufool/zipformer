...
class SwooshL(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.func = torch_compile(_swooshl)
...
class SwooshR(torch.nn.Module):
    def __init__(self):
        super().__init__()
        self.func = torch_compile(_swooshr)
...
class ActivationDropoutAndLinear(torch.nn.Module):
    ...
    def __init__(
        ...
        if activation == "SwooshL":
            self.forward_func = torch_compile(_swooshl)
            self.backward_func = torch_compile(_swooshl_and_deriv)
        else:
            self.forward_func = torch_compile(_swooshr)
            self.backward_func = torch_compile(_swooshr_and_deriv)
